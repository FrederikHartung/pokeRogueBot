...
usage() {
  cat <<'EOF'
Usage:
  scripts/01-data-generation/pipeline/run-wave-library-bootstrap-remote.sh start [config_path]
  ...
  scripts/01-data-generation/pipeline/run-wave-library-bootstrap-remote.sh last
...
run_preflight_checks() {
  ...
  echo "Running iterative pipeline prepare-only validation..."
  (
    cd "${REPO_ROOT}"
    node scripts/01-data-generation/pipeline/run-wave-library-iterative-pipeline.ts "${config_path}" --prepare-only
  )
}
...
start_pipeline() {
  ...
  local cleanup_command="status=\$?; if [ -f '${TELEGRAM_CONTROL_AUTO_FILE}' ]; then bash scripts/01-data-generation/pipeline/run-wave-library-bootstrap-remote.sh telegram-control-stop >/dev/null 2>&1 || true; rm -f '${TELEGRAM_CONTROL_AUTO_FILE}'; fi; exit \$status"
  ...
  if kill -0 "${pid}" >/dev/null 2>&1; then
    ...
    echo "  scripts/01-data-generation/pipeline/run-wave-library-bootstrap-remote.sh logs"
  else
    echo "Pipeline process exited immediately. Check log: ${log_file}" >&2
    if [ -f "${TELEGRAM_CONTROL_AUTO_FILE}" ]; then
      bash scripts/01-data-generation/pipeline/run-wave-library-bootstrap-remote.sh telegram-control-stop >/dev/null 2>&1 || true
      rm -f "${TELEGRAM_CONTROL_AUTO_FILE}"
    fi
    exit 1
  fi
...
telegram_control_start_internal() {
  ...
  nohup bash -lc "export PATH='${VENV_BIN_DIR}':\"\$PATH\" && source '${TELEGRAM_ENV_FILE}' && cd '${REPO_ROOT}' && node scripts/04-automation/telegram/run-telegram-control-bot.mjs --poll-interval-seconds '${poll_interval_seconds}'" \
