...
const runId = `modifier-pokemon-friendship-booster-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-pokemon-friendship-booster-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-pokemon-friendship-booster-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-pokemon-friendship-booster-mask-test.json");
...
console.log("Validated friendship booster items:", summary.validated_items.length);
