...
const runId = `modifier-species-stat-booster-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-species-stat-booster-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-species-stat-booster-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-species-stat-booster-mask-test.json");
...
console.log("Validated species stat booster items:", summary.validated_items.length);
