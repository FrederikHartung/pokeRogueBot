...
const templatePath = path.join(__dirname, "templates", "modifier-memory-mushroom-mask.test.template.ts");
...
const tempTestRelativePath = path.join("test", ".external-rl", runId, "modifier-memory-mushroom-mask.test.ts");
const tempTestPath = path.join(pokerogueRoot, tempTestRelativePath);

const defaultOutputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-memory-mushroom-mask-test.json");
...
if (!existsSync(templatePath)) {
  throw new Error(`Memory mushroom mask template not found: ${templatePath}`);
}
...
if (existsSync(outputPath)) {
  ...
  console.log("Modifier memory mushroom mask output:", outputPath);
  console.log("Action summary:", payload);
}
