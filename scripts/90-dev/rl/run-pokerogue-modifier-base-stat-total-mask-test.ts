...
const runId = `modifier-base-stat-total-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-base-stat-total-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-base-stat-total-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-base-stat-total-mask-test.json");
...
console.log("Validated base stat total modifier variants:", summary.validated_variants.length);
console.log("Variants:", summary.validated_variants.join(", "));
