...
import type { DamageMoneyRewardModifier } from "#modifiers/modifier";
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as DamageMoneyRewardModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as DamageMoneyRewardModifier;
...
it("masks GOLDEN_PUNCH only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.GOLDEN_PUNCH();
  const existingModifier = modifierType.newModifier(cappedTarget) as DamageMoneyRewardModifier;
  ...
  expect(targets[0]?.max_stack_count).toBe(5);
  ...
  validatedItems.push("GOLDEN_PUNCH");
});

it("writes golden punch validation summary", () => {
  ...
  expect(validatedItems).toEqual(["GOLDEN_PUNCH"]);
});
