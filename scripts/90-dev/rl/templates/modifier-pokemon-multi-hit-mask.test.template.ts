...
import type { PokemonMultiHitModifier } from "#modifiers/modifier";
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as PokemonMultiHitModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as PokemonMultiHitModifier;
...
it("masks MULTI_LENS only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.MULTI_LENS();
  const existingModifier = modifierType.newModifier(cappedTarget) as PokemonMultiHitModifier;
  ...
  expect(option?.type.constructor.name).toBe("PokemonMultiHitModifierType");
  ...
  validatedItems.push("MULTI_LENS");
});

it("writes multi-hit booster validation summary", () => {
  ...
  expect(validatedItems).toEqual(["MULTI_LENS"]);
});
