...
import type { PokemonFriendshipBoosterModifier } from "#modifiers/modifier";
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as PokemonFriendshipBoosterModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as PokemonFriendshipBoosterModifier;
...
it("masks SOOTHE_BELL only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.SOOTHE_BELL();
  const existingModifier = modifierType.newModifier(cappedTarget) as PokemonFriendshipBoosterModifier;
  ...
  expect(option?.type.constructor.name).toBe("PokemonFriendshipBoosterModifierType");
  ...
  validatedItems.push("SOOTHE_BELL");
});

it("writes friendship booster validation summary", () => {
  ...
  expect(validatedItems).toEqual(["SOOTHE_BELL"]);
});
