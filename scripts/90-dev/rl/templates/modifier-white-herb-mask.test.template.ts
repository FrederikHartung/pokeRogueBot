...
import type { ResetNegativeStatStageModifier } from "#modifiers/modifier";
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as ResetNegativeStatStageModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as ResetNegativeStatStageModifier;
...
it("masks WHITE_HERB only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.WHITE_HERB();
  const existingModifier = modifierType.newModifier(cappedTarget) as ResetNegativeStatStageModifier;
  ...
  expect(targets[0]?.max_stack_count).toBe(2);
  ...
  validatedItems.push("WHITE_HERB");
});

it("writes white herb validation summary", () => {
  ...
  expect(validatedItems).toEqual(["WHITE_HERB"]);
});
