...
import type { PokemonExpBoosterModifier } from "#modifiers/modifier";
...
const pokemonExpBoosterCases: PokemonExpBoosterCase[] = [
  { itemName: "LUCKY_EGG", typeFactory: () => modifierTypes.LUCKY_EGG() },
  { itemName: "GOLDEN_EGG", typeFactory: () => modifierTypes.GOLDEN_EGG() },
];
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as PokemonExpBoosterModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as PokemonExpBoosterModifier;
...
it.each(pokemonExpBoosterCases)("masks %s only when the target already reached the stack cap", async ({ itemName, typeFactory }) => {
  ...
  const modifierType = typeFactory();
  const existingModifier = modifierType.newModifier(cappedTarget) as PokemonExpBoosterModifier;
  ...
  expect(option?.type.constructor.name).toBe("PokemonExpBoosterModifierType");
...
it("writes pokemon exp booster validation summary", () => {
  ...
  expect(validatedItems).toEqual(pokemonExpBoosterCases.map(testCase => testCase.itemName));
});
