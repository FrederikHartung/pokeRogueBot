...
import type { TurnHeldItemTransferModifier } from "#modifiers/modifier";
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as TurnHeldItemTransferModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as TurnHeldItemTransferModifier;
...
it("masks MINI_BLACK_HOLE only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.MINI_BLACK_HOLE();
  const existingModifier = modifierType.newModifier(cappedTarget) as TurnHeldItemTransferModifier;
  ...
  expect(option?.type.constructor.name).toBe("TurnHeldItemTransferModifierType");
  ...
  expect(targets[1]?.stack_count).toBe(0);
  expect(targets[1]?.available).toBe(true);
  expect(targets[2]?.stack_count).toBe(0);
  expect(targets[2]?.available).toBe(true);
  expect(actionMask).toEqual([0, 1, 1]);

  validatedItems.push("MINI_BLACK_HOLE");
});

it("writes turn held item transfer validation summary", () => {
  ...
  expect(validatedItems).toEqual(["MINI_BLACK_HOLE"]);
});
