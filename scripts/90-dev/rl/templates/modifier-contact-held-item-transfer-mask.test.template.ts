...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as ContactHeldItemTransferChanceModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as ContactHeldItemTransferChanceModifier;
...
it("masks GRIP_CLAW only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.GRIP_CLAW();
  const existingModifier = modifierType.newModifier(cappedTarget) as ContactHeldItemTransferChanceModifier;
  ...
  expect(option?.type.constructor.name).toBe("ContactHeldItemTransferChanceModifierType");
  ...
  validatedItems.push("GRIP_CLAW");
});

it("writes contact held item transfer validation summary", () => {
  ...
  expect(validatedItems).toEqual(["GRIP_CLAW"]);
});
