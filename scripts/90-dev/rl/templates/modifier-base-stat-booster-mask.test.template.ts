...
import type { BaseStatModifier } from "#modifiers/modifier";
...
const baseStatBoosterCases: BaseStatBoosterCase[] = [
  { itemName: "HP_UP", stat: Stat.HP },
  { itemName: "PROTEIN", stat: Stat.ATK },
  { itemName: "IRON", stat: Stat.DEF },
  { itemName: "CALCIUM", stat: Stat.SPATK },
  { itemName: "ZINC", stat: Stat.SPDEF },
  { itemName: "CARBOS", stat: Stat.SPD },
];
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  const dummyModifier = option.type.newModifier(pokemon) as BaseStatModifier;
...
it.each(baseStatBoosterCases)("masks %s only when the target already reached the stack cap", async ({ itemName, stat }) => {
  ...
  const modifierType = modifierTypes.BASE_STAT_BOOSTER().generateType(scene.getPlayerParty(), [stat]);
  if (!modifierType) {
    throw new Error(`Failed to generate base stat booster for ${itemName}`);
  }

  const existingModifier = modifierType.newModifier(cappedTarget) as BaseStatModifier;
  ...
  expect(option?.type.constructor.name).toBe("BaseStatBoosterModifierType");
...
it("writes base stat booster validation summary", () => {
  ...
  expect(validatedItems).toEqual(baseStatBoosterCases.map(testCase => testCase.itemName));
});
