...
import type { PokemonMoveAccuracyBoosterModifier } from "#modifiers/modifier";
...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as PokemonMoveAccuracyBoosterModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as PokemonMoveAccuracyBoosterModifier;
...
it("masks WIDE_LENS only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.WIDE_LENS();
  const existingModifier = modifierType.newModifier(cappedTarget) as PokemonMoveAccuracyBoosterModifier;
  ...
  expect(option?.type.constructor.name).toBe("PokemonMoveAccuracyBoosterModifierType");
  ...
  validatedItems.push("WIDE_LENS");
});

it("writes move accuracy booster validation summary", () => {
  ...
  expect(validatedItems).toEqual(["WIDE_LENS"]);
});
