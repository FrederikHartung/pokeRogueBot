...
function buildTargetSnapshot(pokemon: any, option: ModifierTypeOption, targetPartyIndex: number): TargetSnapshot {
  ...
  ) as PokemonInstantReviveModifier | undefined;
  const dummyModifier = option.type.newModifier(pokemon) as PokemonInstantReviveModifier;
...
it("masks REVIVER_SEED only when the target already reached the stack cap", async () => {
  ...
  const modifierType = modifierTypes.REVIVER_SEED();
  const existingModifier = modifierType.newModifier(cappedTarget) as PokemonInstantReviveModifier;
  ...
  validatedItems.push("REVIVER_SEED");
});

it("writes reviver seed validation summary", () => {
  ...
  expect(validatedItems).toEqual(["REVIVER_SEED"]);
});
