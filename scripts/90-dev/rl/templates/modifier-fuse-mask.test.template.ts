...
it("filters DNA_SPLICERS out of the action mask until fusion support exists", async () => {
  initSceneWithoutEncounterPhase(scene, [SpeciesId.BULBASAUR, SpeciesId.CHARMANDER, SpeciesId.SQUIRTLE]);

  const forcedReward = new ModifierTypeOption(modifierTypes.DNA_SPLICERS(), 0);
  ...
  expect(forcedReward.type.constructor.name).toBe("FusePokemonModifierType");
