...
const runId = `modifier-scope-lens-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-scope-lens-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-scope-lens-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-scope-lens-mask-test.json");
...
console.log("Validated scope lens items:", summary.validated_items.length);
