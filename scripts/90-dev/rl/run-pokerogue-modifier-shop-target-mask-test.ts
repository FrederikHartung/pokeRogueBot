...
const templatePath = path.join(__dirname, "templates", "modifier-shop-target-mask.test.template.ts");
...
const tempTestRelativePath = path.join("test", ".external-rl", runId, "modifier-shop-target-mask.test.ts");
const tempTestPath = path.join(pokerogueRoot, tempTestRelativePath);

const defaultOutputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-shop-target-mask-test.json");
...
if (!existsSync(templatePath)) {
  throw new Error(`Shop target-mask template not found: ${templatePath}`);
}
...
if (existsSync(outputPath)) {
  ...
  console.log("Modifier shop target-mask test output:", outputPath);
  console.log("Potion targets:", payload.potion_targets ?? []);
  console.log("Revive targets:", payload.revive_targets ?? []);
}
