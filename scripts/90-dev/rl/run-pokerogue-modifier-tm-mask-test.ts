...
const runId = `modifier-tm-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-tm-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-tm-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-tm-mask-test.json");
...
console.log("Validated TM mask items:", summary.validated_items.join(", "));
