...
const runId = `modifier-base-stat-booster-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-base-stat-booster-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-base-stat-booster-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-base-stat-booster-mask-test.json");
...
console.log("Validated base stat booster items:", summary.validated_items.length);
