...
const runId = `modifier-pokemon-multi-hit-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-pokemon-multi-hit-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-pokemon-multi-hit-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-pokemon-multi-hit-mask-test.json");
...
console.log("Validated multi-hit booster items:", summary.validated_items.length);
