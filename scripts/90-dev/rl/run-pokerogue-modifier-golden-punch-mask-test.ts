...
const runId = `modifier-golden-punch-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-golden-punch-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-golden-punch-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-golden-punch-mask-test.json");
...
console.log("Validated golden punch items:", summary.validated_items.length);
