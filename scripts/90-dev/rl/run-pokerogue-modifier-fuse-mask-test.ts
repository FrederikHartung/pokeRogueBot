...
const runId = `modifier-fuse-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-fuse-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-fuse-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-fuse-mask-test.json");
