...
const runId = `modifier-form-change-item-group1-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-form-change-item-group1-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-form-change-item-group1-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-form-change-item-group1-mask-test.json");
...
console.log("Validated group-1 form change items:", summary.validated_items.length);
console.log("Item names:", summary.form_change_item_names.join(", "));
