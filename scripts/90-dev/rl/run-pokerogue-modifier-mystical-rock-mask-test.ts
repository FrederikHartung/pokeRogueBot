...
const runId = `modifier-mystical-rock-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-mystical-rock-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-mystical-rock-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-mystical-rock-mask-test.json");
...
console.log("Validated mystical rock checks:", summary.validated_items.length);
console.log("Check names:", summary.validated_items.join(", "));
