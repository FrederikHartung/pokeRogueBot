...
const runId = `modifier-reviver-seed-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-reviver-seed-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-reviver-seed-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-reviver-seed-mask-test.json");
...
console.log("Validated reviver seed items:", summary.validated_items.length);
