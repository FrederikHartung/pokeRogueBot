...
const runId = `modifier-evolution-item-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-evolution-item-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-evolution-item-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-evolution-item-mask-test.json");
...
try {
  ...
  // ignore cleanup issues in temp area
}
...
console.log("Validated evolution items:", summary.validated_items.length);
console.log("Evolution item names:", summary.evolution_item_names.join(", "));
