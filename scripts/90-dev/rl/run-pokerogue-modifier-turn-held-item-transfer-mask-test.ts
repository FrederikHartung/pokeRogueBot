...
const runId = `modifier-turn-held-item-transfer-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-turn-held-item-transfer-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-turn-held-item-transfer-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-turn-held-item-transfer-mask-test.json");
...
console.log("Validated turn held item transfer items:", summary.validated_items.length);
