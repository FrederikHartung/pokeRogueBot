...
const runId = `modifier-white-herb-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-white-herb-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-white-herb-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-white-herb-mask-test.json");
...
console.log("Validated white herb items:", summary.validated_items.length);
