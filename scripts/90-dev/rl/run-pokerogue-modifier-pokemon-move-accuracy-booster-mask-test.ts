...
const runId = `modifier-pokemon-move-accuracy-booster-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-pokemon-move-accuracy-booster-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-pokemon-move-accuracy-booster-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-pokemon-move-accuracy-booster-mask-test.json");
...
console.log("Validated move accuracy booster items:", summary.validated_items.length);
