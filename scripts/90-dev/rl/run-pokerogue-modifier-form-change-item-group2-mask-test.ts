...
const runId = `modifier-form-change-item-group2-mask-${Date.now()}-${randomUUID().slice(0, 8)}`;
const externalRlDir = path.join(pokerogueRoot, "test", ".external-rl", runId);
const templatePath = path.join(__dirname, "templates", "modifier-form-change-item-group2-mask.test.template.ts");
const testPath = path.join(externalRlDir, "modifier-form-change-item-group2-mask.test.ts");
const outputPath = path.join(repoRoot, "data", "temp", "rl", "modifier-form-change-item-group2-mask-test.json");
...
console.log("Validated group-2 form change items:", summary.validated_items.length);
