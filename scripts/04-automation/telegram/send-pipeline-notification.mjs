...
function buildMessage({ event, runtimeDir, manifestPath, phase, iteration, error, manifest, benchmarkSummary, collectionMetrics }) {
  ...
  if (event === "iteration_completed") {
    ...
    if (collectionMetrics) {
      ...
      lines.push(`Transitions: ${collectionMetrics.dataset_rows ?? "?"}`);
      if (collectionMetrics.dataset_size_mb != null) {
        lines.push(`Dataset size: ${formatMegabytes(collectionMetrics.dataset_size_mb)}`);
      }
      if (collectionMetrics.archive_size_mb != null) {
        lines.push(`Archive size: ${formatMegabytes(collectionMetrics.archive_size_mb)}`);
      }
...
function formatMegabytes(value) {
  return typeof value === "number" && Number.isFinite(value) ? `${value.toFixed(2)} MB` : "n/a";
}
