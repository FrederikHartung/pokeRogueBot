...
/**
 * Serializable interface for states that can be logged for training.
 */
interface SerializableState {
    fun toMap(): Map<String, Any?>
    companion object {
        fun fromMap(map: Map<String, Any?>): SerializableState {
            throw NotImplementedError("Must be implemented by concrete state classes")
        }
    }
}

/**
 * Serializable interface for actions that can be logged for training.
 */
interface SerializableAction {
    val actionId: Int
    companion object {
        fun fromId(id: Int): SerializableAction? {
            throw NotImplementedError("Must be implemented by concrete action classes")
        }
 ...
 *
 * @param TState The type of state representation that implements SerializableState
 * @param TAction The type of action that implements SerializableAction
 */
class BaseDecisionLogger<TState, TAction>(
    ...
    private val domainName: String, // e.g., "modifier", "combat"
    private val stateSize: Int,
    private val actionCount: Int,
    private val stateDeserializer: (Map<String, Any?>) -> TState,
    private val actionDeserializer: (Int) -> TAction?
) where TState : SerializableState, TAction : SerializableAction {
    
    private val logger = LoggerFactory.getLogger(BaseDecisionLogger::class.java)
    private val gson = GsonBuilder().setPrettyPrinting().create()
    private val experiences = ConcurrentLinkedQueue<BaseExperience<TState, TAction>>()
    ...
    fun logDecision(
        ...
        val experience = BaseExperience(state, action, reward, nextState, done)
    ...
    fun saveTrainingBatch(sessionId: String? = null): Path? {
        ...
        return try {
            ...
            val fileName = "${sessionPrefix}${domainName}_experiences_${timestamp}.json"
            val filePath = fileSystem.resolve(outputDirectory, fileName)

            val serializedExperiences = experienceList.map { exp ->
                mapOf(
                    "state" to exp.state.toMap(),
                    "action" to exp.action.actionId,
                    "reward" to exp.reward,
                    "nextState" to exp.nextState?.toMap(),
                    "done" to exp.done,
     ...
     */
    fun loadExperiences(filePath: Path): List<BaseExperience<TState, TAction>> {
        return try {
            ...
            val loadedExperiences = experienceData.mapNotNull { expMap ->
                try {
                    val state = stateDeserializer(expMap["state"] as Map<String, Any?>)
                    val actionId = (expMap["action"] as Number).toInt()
                    val action = actionDeserializer(actionId) ?: return@mapNotNull null
                    val reward = (expMap["reward"] as Number).toDouble()
                    val nextState = (expMap["nextState"] as Map<String, Any?>?)?.let(stateDeserializer)
                    val done = expMap["done"] as Boolean
                    val timestamp = (expMap["timestamp"] as Number).toLong()

                    BaseExperience(state, action, reward, nextState, done, timestamp)
                } catch (e: Exception) {
                    logger.warn("Failed to deserialize experience: ${e.message}")
                    null
                }
            }
            
            logger.info("Loaded ${loadedExperiences.size} experiences from $filePath")
            loadedExperiences
        } catch (e: Exception) {
     ...
     */
    fun getExperienceBuffer(): List<BaseExperience<TState, TAction>> {
