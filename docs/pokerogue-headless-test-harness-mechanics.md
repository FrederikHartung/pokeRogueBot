...
- ~~`advanceCombatAfterAction` pumpt `SwitchSummonPhase` zusaetzlich einmal isoliert (`await game.phaseInterceptor.to("SwitchSummonPhase")`).~~ **Wieder entfernt** nach dem Fix aus 3.11: per TDD geprueft (Guard einzeln entfernt, Regressionstest mit installiertem Single-Driver-Schutz weiter gruen) - die Pumpe war nur wegen konkurrierender `to()`-Treiber noetig und konnte ihrerseits einer bereits gelaufenen `SwitchSummonPhase` hinterherjagen. Der Guard in `advanceCurrentUiPromptIfPossible` dagegen wird **weiterhin gebraucht** (ohne ihn scheitert der Regressionstest auch mit Single-Driver-Schutz).
...
**Der Fix, Teil 1** (`resolveForcedSwitchIfNeeded` in `test/porubot/harness/battle-command-advance.ts`): erkannte den erzwungenen Wechsel zusaetzlich ueber `ui_mode === PARTY && partyUiMode === PartyUiMode.FAINT_SWITCH`, nicht mehr nur ueber `isCurrentPhase("SwitchPhase")`. **Wieder entfernt** nach dem Fix aus 3.11: `Pokemon.switchOut()` wird nur von `TurnInitPhase.start()` aufgerufen, und legitim nur bei echten Challenge-Verstoessen - ein normaler K.O. laeuft immer ueber `FaintPhase` → `SwitchPhase`. Der beobachtete phasenlose Party-UI-Fall war eine Folge einer **uebersprungenen `FaintPhase`** durch doppelt gestartete Phasen (3.11). Der Collector spielt nur Classic-Modus ohne Challenges; ein erneutes Auftreten soll daher als sichtbarer Timeout auffallen, statt still weggefangen zu werden. Der Punkt 2 oben (Mechanismus von `switchOut()`) bleibt als Wissen gueltig.
...
Der Collector (`scripts/90-dev/rl/templates/modifier-fixed-seed-collector.test.template.ts`) ruft `installSingleDriverPhaseInterceptor(game)` direkt nach `new GameManager(...)` auf, ebenso `battle-command-advance.test.ts` im `beforeEach`, damit die Harness-Tests exakt wie in der Pipeline laufen.

**Nachpruefung der Schutzmassnahmen aus 3.8-3.10** (per TDD, jede einzeln entfernt und mehrfach getestet): die `SwitchSummonPhase`-Pumpe (3.8) und die phasenlose `PARTY`/`FAINT_SWITCH`-Erkennung (3.10 Teil 1) waren nur Symptom-Behandlungen dieses Mechanismus und wurden entfernt. Die beiden Ausnahmen vom blinden ACTION-Druck in `advanceCurrentUiPromptIfPossible` (`SwitchSummonPhase`, `SwitchPhase`) sind dagegen ein **eigenstaendiger** Mechanismus (Polling-Interferenz, unabhaengig von mehreren Treibern) und bleiben. **Regel fuer neuen Collector-/Harness-Code:** jeder Code, der `to()`-Aufrufe ueberlappen oder unerledigt zuruecklassen kann, muss mit installiertem Single-Driver-Schutz laufen.
